...
import edu.buffalo.qa.servlet.Tagger;
	...
	protected void doGet(HttpServletRequest request,
		...
		//	String userInput = (String) request.getParameter("question"); 
			...
			/*if(userInput.isEmpty())
				return;   // Check this logic for erro handling
			*/
			...
			//params.set("qf", "name3 description^10");
			...
			System.out.println(" Hiiiiiiiiiiii  Count of Returned values:" + docs.size());
			...
			if(size==1 & bool_Single==false)
			{
				...
				System.out.print("description : " +ansDesc);
					 ...
					 ans="Unfortunately system doesn't know about "+ "'" +ansType + "' of '" + properNoun +"' ; try something else about " + properNoun + "  !!.." ;  
			...
			//	str= str+doc.getFieldValue(ansType).toString();
			  ...
			  if(doc.getFieldValue("name")!=null)	
				...
				for(SolrDocument d : docs){					 
					 ...
					 ans="Unfortunately system doesn't know about "+ "'" +ansType + "' of '" + properNoun +"' ; try searching something else about " + properNoun + "  !!.." ;  
			...
			else if	(size==0 && null!=sresp && bool_Single==false)
			{
				//spellSuggest="Did you mean :"+ "\n";
				   ...
				   // Suggestion s   = entry.getValue();				    
				    ...
				    {
				    	if(!bool){
				      	String st=orgQues.replace(replacewith," "+c.getCollationQueryString()+" ");
				       	str= st ;
				...
				if(size==0)
				{
					HttpSolrServer serverPlace = new HttpSolrServer(orgUrl+"places");					
					...
					{
					HttpSolrServer serverFilms = new HttpSolrServer(orgUrl+"films");					
				...
				for(String s : fields)
				{
					if((!d.getFieldValue(s).toString().isEmpty()) && !s.equalsIgnoreCase("description")){
						String correctVal= d.getFieldValue(s).toString() ;
						correctVal=correctVal.replace("[", "");
						correctVal=correctVal.replace("]", "");
					    request.setAttribute("Field"+count,s+"="+correctVal);
					    System.out.println("Field"+count +"," +s+"="+correctVal);
					    count++;						
					}
					if(count==6)
						break;
				  ...
				  correctdes=correctdes.replace("]", "");
				  request.setAttribute("Field"+count,"description="+correctdes);
				  System.out.println("Field"+count+" description="+correctdes);
			...
			System.out.println("suggest: " +ansSuggestions);
		    String address = "";
